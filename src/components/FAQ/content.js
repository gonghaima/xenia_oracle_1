export const data = [
    {
        key: 'faq-data-1',
        title: 'How easy is Xenia Suite to use?',
        content: 'Xenia Suite is designed to make your life easier. We can implement and train your team on Xenia Suite in a matter of days, taking care of the day to day so you can concentrate on what matters most.'
    },
    {
        key: 'faq-data-2',
        title: 'Is local support available?',
        content: 'We have two New Zealand support centres located in Auckland and Queenstown. Once you’re part of the Xenia Suite family we offer 24/7 phone and online support, as well as on-site training as required.'
    },
    {
        key: 'faq-data-3',
        title: 'Can Xenia Suite be used on a tablet?',
        content: 'Xenia Suite has a full range of mobile applications including Reception, Housekeeping and Laundry, Maintenance, Minibar and a Guest Application that can run on tablets or smartphones.'
    },
    {
        key: 'faq-data-4',
        title: 'Does Xenia Suite integrate with a fully featured accounting system?',
        content: 'Xenia Suite comes with fully integrated MYOB Exo Accounting and CRM. It is designed as an end-to-end system - from Bookings, Front-Desk and Point of Sale through to Operations and Function Management, Financial Management and Reporting.'
    },
    {
        key: 'faq-data-5',
        title: 'Does Xenia Suite have to be in the cloud?',
        content: 'We will configure Xenia Suite to run on the platform that suits you. Be that in-house or in the cloud.'
    },
    {
        key: 'faq-data-6',
        title: 'Can Xenia Suite grow as my business grows?',
        content: 'Yes, absolutely! Xenia Suite is configured for mid-range properties (50 rooms plus) and is scalable and suitable for larger sites up to 130 rooms. Xenia Suite also offers multiple property functionality that can enable individual or centralised back- office accounting.'
    },
    {
        key: 'faq-data-7',
        title: 'How much does Xenia Suite cost?',
        content: 'Your hotel management system is an integral part of your business, and we understand you want to invest in the right system for you. At Xenia Suite we offer flexible pricing options based on the size of your property and your occupancy rates. Contact us today for a no- obligation quote.'
    }
];

export const activities = [
    {
        key: 'faq-activity-1',
        heading: 'Front of House',
        details: [
            {
                key: 'faq-activity-1-1',
                title: 'Can my team use mobile front-desk functionality to check in guests away from a physical front desk?',
                content: 'Yes, absolutely! Xenia Suite is designed to take the stress out of the every day. In just two clicks of a button your guests will be checked in, and ready to head to their rooms or prop up a table in your bar!'
            },
            {
                key: 'faq-activity-1-2',
                title: 'Can guests self-manage their booking?',
                content: 'Yes, Xenia Suite’s mobile Guest App enables guests to self-check-in, check-out and manage their booking preferences before, during and after their stay.'
            }
        ]
    },
    {
        key: 'faq-activity-2',
        heading: 'Bookings/Reservation Management',
        details: [
            {
                key: 'faq-activity-2-1',
                title: 'Does Xenia Suite integrate with OTAs?',
                content: 'Yes, Xenia Suite offers integrated online bookings, either direct from your hotel website or through a booking portal.'
            },
            {
                key: 'faq-activity-2-2',
                title: 'Can Xenia Suite help with groups and tour bookings?',
                content: 'This is Xenia Suite’s time to shine! Whether your guests arrive together or separately, Xenia Suite has you covered. With group and single payment options, and the ability to link bookings to those outside the group, Xenia Suite is here to help make the process as smooth as possible.'
            },
            {
                key: 'faq-activity-2-3',
                title: 'How does Xenia Suite handle overbookings?',
                content: 'Need to bump rooms or accommodate a late booking? Xenia Suite is here to help you easily manage and move your bookings and features an auto PO generator for your partner hotels as and when you need it.'
            },
            {
                key: 'faq-activity-2-4',
                title: 'Can Xenia Suite help me manage functions and events?',
                content: 'Yes, of course! With one easy-to-use screen, you can manage function dates, rooms, attendees, equipment and catering, and communicate information with your team with seamless integration.'
            }
        ]
    },
    {
        key: 'faq-activity-3',
        heading: 'Housekeeping',
        details: [
            {
                key: 'faq-activity-3-1',
                title: 'Does Xenia Suite have a housekeeping system?',
                content: 'Yes, Xenia Suite has full Housekeeping functionality and a Mobile app for your team. The app enables scheduling, late check-outs, staff monitoring and performance measurement, checklists, room status, photo attachment capability and live chat so you and the team are working in real - time.'
            }
        ]
    },
    {
        key: 'faq-activity-4',
        heading: 'Minibar Mobile App',
        details: [
            {
                key: 'faq-activity-4-1',
                title: 'Can the Xenia Suite Minibar App auto-charge rooms?',
                content: 'Yes, and the app can also keep your guests notified of minibar charges via their Guest App.'
            },
            {
                key: 'faq-activity-4-2',
                title: 'Does Xenia Suite offer minibar stock management?',
                content: 'Yes, Xenia Suite has full inventory management including forward order planning, stock-outs, and damaged or out of date goods replenishment.'
            }
        ]
    },
    {
        key: 'faq-activity-5',
        heading: 'Guest App and Guest Experience',
        details: [
            {
                key: 'faq-activity-5-1',
                title: 'Can my guests self-check-in/check-out?',
                content: 'Yes, if you activate this function your guest can check-in and check-out using the Xenia Guest App.'
            },
            {
                key: 'faq-activity-5-2',
                title: 'Can guests create booking request?',
                content: 'Yes, your guests are able to book their room and any additional amenities at your hotel before, during and after their stay.'
            },
            {
                key: 'faq-activity-5-3',
                title: 'Can guests view their bill?',
                content: 'Yes, your guests can view their in-room charges via the Xenia Guest app. They can also request for a bill to be emailed to them from a few clicks of a button.'
            },
            {
                key: 'faq-activity-5-4',
                title: 'Can my guests view directions to the hotel on the Guest App?',
                content: 'Yes, the Xenia Suite Guest App can be linked to Google Maps, and can also include an internal map of your hotel if that’s something you would like to offer your guests.'
            },
            {
                key: 'faq-activity-5-5',
                title: 'Can we use the Xenia Suite Guest App to market to our customers?',
                content: 'Yes, using Xenia Suite’s CRM functionality targeted marketing campaigns and a guest push facility can be added to the Xenia Suite Guest App.'
            },
            {
                key: 'faq-activity-5-5',
                title: 'Can my guests book taxis and other services through the Guest App?',
                content: 'Yes, our Guest App has the ability to offer additional amenities at your hotel, this includes taxi’s, spa’s and gym services.'
            },
            {
                key: 'faq-activity-5-7',
                title: 'Can my guests book room service or book a table in my restaurant?',
                content: 'Yes, the Xenia Suite Guest App allows your guests to book a table at your in-house restaurant or order from your room service menu.'
            },
            {
                key: 'faq-activity-5-8',
                title: 'Can I include important information on the Guest App, such as the Wi-Fi code?',
                content: 'Yes, Xenia Suite can permit guests to access information such as Wi-Fi passwords, gate/lock codes or important information that you want to ensure your guests have readily available.'
            }
        ]
    },
    {
        key: 'faq-activity-6',
        heading: 'Restaurant and Point-of-Sale',
        details: [
            {
                key: 'faq-activity-6-1',
                title: 'Does Xenia Suite have a POS system?',
                content: 'Yes, Xenia Suite has an integrated Restaurant and Bar POS system. This integrates with the Guest App so you and your guests have access to real- time information.'
            },
            {
                key: 'faq-activity-6-2',
                title: 'Does Xenia Suite handle boutiques?',
                content: 'Yes, Xenia Suite’s POS functionality enables you to run an -n-house boutique as well as a restaurant.'
            },
            {
                key: 'faq-activity-6-3',
                title: 'Does Xenia Suite include a kitchen printer?',
                content: 'Yes, all orders processed in the restaurant can be automatically sent through to the kitchen and printed out.'
            }
        ]
    },
    {
        key: 'faq-activity-7',
        heading: 'Back-Office; Guest Data, Financial Management and Reporting',
        details: [
            {
                key: 'faq-activity-7-1',
                title: 'Can Xenia Suite keep track of guests’ history and preferences?',
                content: 'Yes, Xenia Suite can securely store guest preferences for an unlimited time period.'
            },
            {
                key: 'faq-activity-7-2',
                title: 'Can Xenia Suite help with marketing campaigns?',
                content: 'Yes, Xenia Suite has a detailed CRM function for targeted email campaigns and guest messaging.'
            },
            {
                key: 'faq-activity-7-3',
                title: 'What accounting system does it need?',
                content: 'Xenia Suite does not need a separate accounting system, as it incorporates MYOB Exo which is a fully functional ERP.'
            },
            {
                key: 'faq-activity-7-4',
                title: 'Can Xenia Suite manage Fixed Assets and major Maintenance and Refurbishments?',
                content: 'Yes, not only can we maintain full oversight through the life of all Fixed Assets but can treat major Maintenance and Refurbishments as discrete projects and provide oversight and management of all timelines and costs.'
            }
        ]
    },
    {
        key: 'faq-activity-8',
        heading: 'Technical',
        details: [
            {
                key: 'faq-activity-8-1',
                title: 'Does Xenia Suite integrate with other systems?',
                content: 'Xenia Suite has a fully functional API and proven integration to Booking Engines and other partner applications. Contact us to find out more information.'
            },
            {
                key: 'faq-activity-8-2',
                title: 'Can Xenia Suite help me with my night audit?',
                content: 'Yes, absolutely. Xenia Suite is designed to save you time and effort and can complete your night audit in a matter of minutes.'
            }
        ]
    },
];